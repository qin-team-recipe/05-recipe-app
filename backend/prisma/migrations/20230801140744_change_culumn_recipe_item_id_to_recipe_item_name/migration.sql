/*
  ...
  - You are about to drop the column `recipe_item_id` on the `shopping_items` table. All the data in the column will be lost.
  ...
  - Added the required column `recipe_item_name` to the `shopping_items` table without a default value. This is not possible if the table is not empty.

*/
-- DropForeignKey
ALTER TABLE `shopping_items` DROP FOREIGN KEY `shopping_items_recipe_item_id_fkey`;
...
-- AlterTable
ALTER TABLE `shopping_items` DROP COLUMN `recipe_item_id`,
    ADD COLUMN `recipe_item_name` VARCHAR(191) NOT NULL,
